...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
    ...
    if (strlen($_POST['publisher_name']) == '') 
    {
            $input_error = 1;
            display_error(_("Publisher name must be entered."));
            set_focus('publisher_name');
    }
    ...
    if ($input_error !=1)
    {
        ...
        if ($selected_id != -1) 
        { 
	    update_publisher($selected_id,$_POST['publisher_name'],$_POST['address'],$filename,$_POST['status']);    	    		
			display_notification(_('Selected data has been updated'));
        } 
        else 
        {
           add_publisher($_POST['publisher_name'],$_POST['address'],$filename,$_POST['status']);    		
           display_notification(_('New Publisher has been added'));
        }
...
if ($Mode == 'Delete')
{
        dynamic_delete('sms_lib_publisher_setup',$selected_id);
               
		display_notification(_('Selected publisher has been deleted'));
...
$newsql=view_publisher();

$cols = array(
    _("Image") => array('insert' => false, 'fun' => 'publisher_image', 'align' => 'center'),
    _("Publisher Name")=>array('align'=>'center'),
    _("Address")=>array('align'=>'center'),
    array('insert'=>true, 'fun'=>'edit_link', 'align'=>'center'),
    array('insert'=>true, 'fun'=>'delete_link', 'align'=>'center')
);
$table = & new_db_pager('sms_lib_publisher_setup', $newsql, $cols);
...
if ($selected_id != -1) 
{
 	...
 	if ($Mode == 'Edit') {
                ...
                $myrow = data_retrieve('sms_lib_publisher_setup', 'id', $selected_id);       
              
                $_POST['publisher_name'] = $myrow['publisher_name'];
		$_POST['address'] = $myrow['address'];
                $filename = $myrow["image"];
                $_POST['status'] = $myrow["status"];
	}
	hidden('id', $selected_id);
}

text_row(_("Publisher Name:"),'publisher_name',$_POST['location_name'],30,30);
textarea_row("Address:", 'address', $_POST['address'], 20, 3);
file_row(_("Publisher Image (.jpg)") . ":", 'pic', 'pic');
