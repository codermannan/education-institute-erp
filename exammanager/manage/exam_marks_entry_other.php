...
if (isset($_POST['Process'])) 
{
	...
	if ($marks == '') 
	{
		...
		set_focus('mark');
            ...
            add_exam_marks($ids,$syear,$_POST['exam_name'],$stclass,$_POST['section'],$_POST['subject'],$stid, $marks,$grade,$gpa,$user);
...
$query=array('id','exam_name','sms_exam_name','parent = 0 AND class_name',$classval['class']);
...
$sql = get_sql_for_students_mark_entry($syear, $class, $subject, $exam_name);
  ...
  while ($rep = db_fetch($result))
    {
         ...
         $markchk = checkmarkentry($rep['student_id'],$syear,$class,$subject,$exam_name,$child_exam_name);
         $chkmarklimit = chkmarklimit($syear,$class,$subject,$exam_name,$child_exam_name);
         ...
         start_row();
             ...
             text_cells($rep['mark'], 'mark['.$rep['id'].']', null, 20,2,false,'','');
