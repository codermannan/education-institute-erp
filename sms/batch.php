...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	if (strlen($_POST['batch_name']) == 0) 
	{
		$input_error = 1;
		display_error(_("The batch name must be entered"));
		set_focus('batch_name');
	}
    	...
    	if ($selected_id != -1) 
    	{ 
            update_batch($selected_id,$_POST['batch_name']);    		
                   
                    display_notification(_('Selected data has been updated'));
    	} 
    	else 
    	{ 
                add_batch($_POST['batch_name']);
...
{
 	if ($Mode == 'Edit') {
                $myrow = data_retrieve("sms_batch", "id", $selected_id);
		$_POST['batch_name'] = $myrow["batch_name"];
		 
	}
	hidden('selected_id', $selected_id);
}

text_row(_("Batch Name:"), 'batch_name', null, 25, 30);
